...
protocol ComposeServiceAlgorithmViewControlling: AnyObject {
    func reload(with data: ComposeServiceAlgorithmMenuSection)
}

final class ComposeServiceAlgorithmViewController: UIViewController {
    var presenter: ComposeServiceAlgorithmPresenter!
    
    private let tableView = SettingsMenuTableView()
    
    private var tableViewAdapter: TableViewAdapter<ComposeServiceAlgorithmMenuSection, ComposeServiceAlgorithmMenuCell>!
...
extension ComposeServiceAlgorithmViewController: ComposeServiceAlgorithmViewControlling {
    func reload(with data: ComposeServiceAlgorithmMenuSection) {
        let snapshot = TableViewDataSnapshot<ComposeServiceAlgorithmMenuSection, ComposeServiceAlgorithmMenuCell>()
        snapshot.appendSection(data)
        tableViewAdapter.apply(snapshot: snapshot)
    }
}

extension ComposeServiceAlgorithmViewController {
    func cell(
        for data: ComposeServiceAlgorithmMenuCell,
        ...
        cell.update(icon: nil, title: data.algorithm.rawValue, kind: accessory)
    ...
    func didSelect(tableView: UITableView, indexPath: IndexPath, data: ComposeServiceAlgorithmMenuCell) {
